...
def lista_de_nomes_e_inicial(lista):
    ...
    """
    return [(palavra[0], palavra) for palavra in lista]
...
def produto_cartesiano_filtro(homens, mulheres):
    ...
    return [
        ...
        if not len(homem) < 4 and not len(mulher) < 4
    ]
...
def pares_e_divisiveis_por_7(limite_inicial=1067, limite_final=3627):
    ...
    return len(
        [
            numero
            for numero in range(limite_inicial, limite_final + 1)
...
def duplica_caracter(s):
    ...
    """
    return "".join([caracter * 2 for caracter in s])
...
def gago(texto):
    ...
    """
    return " ".join([f"{palavra[0]}-{palavra}" for palavra in texto.split(" ")])
...
def explode_string(s):
    ...
    """
    return "".join([s[:index] for index in range(len(s) + 1)])
...
def intercalamento_listas(lista1, lista2):
    ...
    return [numero for numero in zip(lista1, lista2) for numero in numero]
...
def numeros_sortudos(limite_inferior=1, limite_superior=100000):
    ...
    return len(
        [
            ...
            if not "7" in str(numero) and "2" in str(numero)
        ]
    )
